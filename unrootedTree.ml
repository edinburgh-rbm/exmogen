    ...
    *)
    ...
    let canonical_root (tree : t) =
      ...
      | [x] ->
        ...
        (Graph.v_of_int x, R.canonical (root tree vx))
      | [x; y] ->
        let vx = Graph.v_of_int x in
        let vy = Graph.v_of_int y in
        let rx = R.canonical (root tree vx) in
        let ry = R.canonical (root tree vy) in
        let c  = R.compare rx ry in
        if c = 1 then
          (Graph.v_of_int y, ry)
        else
    ...
    let canonical tree = snd (canonical_root tree)

    let compare : canonical -> canonical -> int = R.compare

    let print x = "UnrootedTree.Canonical.print: unimplemented"
